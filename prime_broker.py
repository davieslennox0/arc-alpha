...
from scout_skill import router as scout_router
...
_EARNINGS_FILE = "agent_earnings.json"

def _load_earnings():
    from pathlib import Path
    if Path(_EARNINGS_FILE).exists():
        try:
            with open(_EARNINGS_FILE) as f:
                return _json.load(f)
        except:
            pass
    return {a: 0.0 for a in AGENT_WALLETS}

def _save_earnings():
    with open(_EARNINGS_FILE, "w") as f:
        _json.dump(AGENT_EARNINGS, f)

AGENT_EARNINGS = _load_earnings()
...
def root():
    return {
        ...
        "version": "2.0.0",
        "tiers": {k: f"${v['price_usdt']} USDC" for k, v in FEE_TIERS.items()},
...
def preview(asset: str):
    ...
    return {
        ...
        "direction": "*** PAY $0.01 USDC TO UNLOCK ***",
...
def full_broker(req: FullBrokerRequest):
    ...
    # Scout Agent earns for signal generation
    record_agent_earnings("signal", FEE_TIERS["signal"]["price_usdt"])
...
def agent_status():
    return {
        "agents": [
            ...
            {"name": "Execution", "role": "Uniswap V3 on Arc",  "earnings_usdt": AGENT_EARNINGS["execution"]},
        ],
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("prime_broker:app", host="0.0.0.0", port=8002, reload=False)
...
def mcp_manifest():
    return {
        ...
        "description": "Managed trade execution for AI agents on Arc. Pay x402, get Uniswap V3 execution.",
        ...
        "tools": [
            ...
            {"name": "get_signal", "description": "Full signal. Costs $0.01 USDC.", "parameters": {"type": "object", "properties": {"asset": {"type": "string"}, "tx_hash": {"type": "string"}}, "required": ["asset", "tx_hash"]}},
            {"name": "validate_signal", "description": "Risk validation. Costs $0.02 USDC.", "parameters": {"type": "object", "properties": {"asset": {"type": "string"}, "direction": {"type": "string"}, "confidence": {"type": "number"}, "tx_hash": {"type": "string"}}, "required": ["asset", "direction", "confidence", "tx_hash"]}},
            {"name": "execute_trade", "description": "Uniswap V3 execution. Costs $0.05 USDC.", "parameters": {"type": "object", "properties": {"asset": {"type": "string"}, "direction": {"type": "string"}, "amount_usdt": {"type": "number"}, "agent_id": {"type": "string"}, "tx_hash": {"type": "string"}}, "required": ["asset", "direction", "amount_usdt", "agent_id", "tx_hash"]}},
            {"name": "full_broker", "description": "Full pipeline. Costs $0.02 USDC.", "parameters": {"type": "object", "properties": {"asset": {"type": "string"}, "agent_id": {"type": "string"}, "tx_hash": {"type": "string"}}, "required": ["asset", "agent_id", "tx_hash"]}},
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        try:
            if call.name == "get_preview":
                r = await client.get(f"http://127.0.0.1:8002/preview/{call.parameters['asset']}")
                return {"result": r.json()}
            elif call.name == "get_signal":
                r = await client.post(f"http://127.0.0.1:8002/signal", json=call.parameters)
                return {"result": r.json()}
            elif call.name == "validate_signal":
                r = await client.post(f"http://127.0.0.1:8002/validate", json=call.parameters)
                return {"result": r.json()}
            elif call.name == "execute_trade":
                r = await client.post(f"http://127.0.0.1:8002/execute", json=call.parameters)
                return {"result": r.json()}
            elif call.name == "full_broker":
                r = await client.post(f"http://127.0.0.1:8002/broker", json=call.parameters)
                return {"result": r.json()}
            elif call.name == "get_status":
                r = await client.get(f"http://127.0.0.1:8002/status")
                return {"result": r.json()}
            elif call.name == "get_activity":
                r = await client.get(f"http://127.0.0.1:8002/activity")
                return {"result": r.json()}
...
SCOUT_FEED_FILE = "scout_feed.json"

def load_scout_feed():
    if _Path2(SCOUT_FEED_FILE).exists():
        with open(SCOUT_FEED_FILE) as f:
            return _json2.load(f)
    return []

def save_scout_feed(feed):
    with open(SCOUT_FEED_FILE, "w") as f:
        _json2.dump(feed[-50:], f)

@app.get("/scout/feed")
def get_scout_feed():
    return {"feed": load_scout_feed()[-20:]}

@app.post("/scout/record")
def record_scout_signal(data: dict):
    feed = load_scout_feed()
    data["timestamp"] = int(__import__("time").time())
    feed.append(data)
    save_scout_feed(feed)
    return {"ok": True}
